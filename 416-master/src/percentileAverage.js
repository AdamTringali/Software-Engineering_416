...
var SAT_CSV_PATH = "./SAT Composite Scores.csv"
var SAT_SUBJECT_CSV_PATH = "./SAT Subject Tests.csv"
var ACT_CSV_PATH = "./ACT Scores.csv"
...
function structureStudentProfile(userProfile) {
	userProfile.tests = {
		SAT_subject_tests: {
			SAT_literature: userProfile.SAT_literature,
			SAT_US_hist: userProfile.SAT_US_hist,
			SAT_world_hist: userProfile.SAT_world_hist,
			SAT_math_I: userProfile.SAT_math_I,
			SAT_math_II: userProfile.SAT_math_II,
			SAT_eco_bio: userProfile.SAT_eco_bio,
			SAT_mol_bio: userProfile.SAT_mol_bio,
			SAT_chemistry: userProfile.SAT_chemistry,
			SAT_physics: userProfile.SAT_physics,
		},
		ACT_subject_tests: {
		...
		composite_scores: {
			SAT_math: userProfile.SAT_math,
			SAT_EBRW: userProfile.SAT_EBRW,
...
function percentileAverage(userProfile) {
	...
	userSubjectTests = userProfile.tests.SAT_subject_tests
	...
	composite_scores = userProfile.tests.composite_scores
	if (composite_scores.SAT_EBRW > -1 && composite_scores.SAT_math > -1 && composite_scores.ACT_composite > -1) {
        ...
        SAT_EBRW_percentile = calculatePercentile("SAT_EBRW", composite_scores["SAT_EBRW"])
        SAT_math_percentile = calculatePercentile("SAT_math", composite_scores["SAT_math"])
        final_score += (SAT_EBRW_percentile * SAT_WEIGHT/2) + (SAT_math_percentile * SAT_WEIGHT/2)
    
		ACTpercentile = calculatePercentile("ACT_composite", composite_scores["ACT_composite"])
